...
def draw_smile(sc,c): #рисуем смайлик
    ...
    :return: возвращает элемент в рабочую область
    """
...
def reflection(c): #отражение от стенок
    ...
    :param c: переменная, которая отражает конкретный круг
    :return: при достижении границ рабочей области смайлы возвращаются обратно в нее
    """
    global max_w, max_h
    if c.x - c.r <= 0:
        c.vx = 1
...
def reflection_rect(c):#отражение прямоуг от краёв
    """Метод для осуществления отржаения прямоугольных смайлов от границ рабочей области
